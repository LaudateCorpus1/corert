...
namespace Internal.JitInterface
{
    public class PrimitiveTypeDescriptor
    {
        ...
        public static uint GetPrimitiveTypeIndex(TypeDesc typeDescriptor)
        {
            Debug.Assert(typeDescriptor.IsPrimitive, "it is not primitive type");
            switch (typeDescriptor.Category)
            {
                case TypeFlags.Void:
                ...
                case TypeFlags.IntPtr:
                    return (uint)TYPE_ENUM.T_32PLONG;
                case TypeFlags.UIntPtr:
                    return (uint)TYPE_ENUM.T_32PULONG;
